...
static BLEUUID    MDUUID("12345678-B5A3-F393-E0A9-E50E24DCCA9E"); //Additional unique ID
...
static BLERemoteCharacteristic* MDRemoteCharacteristic;
...
static void notifyCallbackMD(
  BLERemoteCharacteristic* pBLERemoteCharacteristic,
  uint8_t* pData,
  size_t length,
  bool isNotify) {
  dataGeneral = (char*)pData; //converting uint8_t to char array
  dataGeneral[length] = '\0'; //adding end of string
  state = "InGeneral";
}
...
bool connectToServer() {
  ...
  MDRemoteCharacteristic = pRemoteService->getCharacteristic(MDUUID);
  Serial.println(RRemoteCharacteristic->getUUID().toString().c_str());
    Serial.println(MDRemoteCharacteristic->getUUID().toString().c_str());
  Serial.println(" - Found our characteristic");
  // Read the value of the characteristic.
//  if (MDRemoteCharacteristic->canNotify()) {
  //  MDRemoteCharacteristic->registerForNotify(notifyCallbackMD);
  //}
...
void drawGraph() {
  ...
  JSONVar myObject = JSON.parse(dataGeneral);
  Serial.println(myObject);
  Serial.println("Should be printing something here");
//{"data":{"training":[],"data":[200,-101]}}

//{'data': [1024, -51], 'training': [(31, 5)]}

  int light= myObject["data"]["data"][0];
  int motor= myObject["data"]["data"][1];
  GRAPH(light,motor);
}
...
void loop() {
  ...
  else if (state == "InStream" &&  connected && screen=="Stream") {
    state = "";
  ...
  else if (state == "InGeneral" &&  connected && screen=="General") {
    state = "";
    drawGraph();
  }
