...
void moveMotor(int sensorValue){
  half=abs(sensorValue/2-1);
  diff=256-half;
  digitalWrite(motor1pin1, (diff>=0));
  digitalWrite(motor1pin2, (diff<0));
  analogWrite(enable,abs(diff));
}
...
void loop() {
  ...
  moveMotor(sensorValue); //change here for servo
  ...
  if(SMMode=='T'){
    ...
    while(!digitalRead(switchPin)){
      ...
      moveMotor(pos); //change here for servo 
      delay(100);
      }
  dataCount=0;
  }
